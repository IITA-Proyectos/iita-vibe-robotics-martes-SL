...
const unsigned long MS_OSCURO_ANTES   =  1500;  // LED apagado ANTES de contar,
                                                // para que el anuncio no se
                                                // confunda con el parpadeo del
                                                // final de la cuenta inicial
...
void loop() {
  ...
  switch (fase) {
    ...
    case ANUNCIO: {
      ...
      //
      // ARREGLO DEL DELANTERO [2026-08-25]. En la version del arquero el
      // anuncio arrancaba PEGADO al parpadeo rapido del final de la cuenta
      // inicial, y el destello unico de la posicion 1 se confundia con el.
      // A la mesa del arquero se le corrio TODA la tabla un lugar por eso
      // (ver su bitacora 2026-08-25-tabla-de-conversion-de-la-camara.md, que
      // lo cuenta y lo salva leyendo la estructura de los numeros).
      // La solucion son MS_OSCURO_ANTES ms de LED APAGADO antes de empezar a
      // contar: el silencio separa una cosa de la otra sin ambiguedad.
      unsigned long transcurrido = ahora - t_fase;
      if (transcurrido < MS_OSCURO_ANTES) { digitalWrite(LED, LOW); break; }
      transcurrido -= MS_OSCURO_ANTES;
