/* #####################################################################
   RESPALDO — PROGRAMA DEL PARTIDO 1 (2026-09-21) — GANO 5 A 0
   #####################################################################
   NO SE TOCA. Es la copia de seguridad de lo que estaba cargado en el
   delantero en el primer partido del 21/09 (jugando con el arquero de
   la otra mesa). Se sigue trabajando en ../../funciona/delantero/.

   Valores con los que jugo:
     patada            215 x 420 ms (rampa +15 cada 5 ms)
     al ver la linea   retrocede 250 ms a 200, sin mirar la linea 1000 ms
     orbita            Xp<34, impulso 120 x 300 ms -> crucero 67 (max 20 s)
     avance            95
     arranque          cuenta regresiva de 2 s

   Lo unico agregado respecto de lo que se jugo es este comentario y una
   linea del banner que dice que es el respaldo. Ninguna de las dos
   cambia como juega el robot.

   PARA CARGARLO: desde ESTA carpeta, con la app Teensy Loader abierta y
   un solo robot enchufado:   pio run -e teensy41 -t upload
   Y confirmar en el monitor (19200) que el banner diga
   "RESPALDO PARTIDO 1".
   ##################################################################### */
...
const unsigned long MS_RETROCESO_LINEA = 250;   // cuanto retrocede. 300 (15/09) -> 200 -> 250 (21/09, pedido del equipo)
...
void setup() {
  ...
  Serial.println();
  Serial.println("==============================================");
  Serial.println("*** RESPALDO PARTIDO 1 (21/09, gano 5 a 0) ***");
