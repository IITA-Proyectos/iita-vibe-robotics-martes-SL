...
def guardar_calibracion():
    ...
    try:
        with open("calibracion_rolling_window.json", "w") as f:
            json.dump(datos, f)
...
def cargar_calibracion():
    ...
    try:
        with open("calibracion_rolling_window.json", "r") as f:
            datos = json.load(f)
            for i in range(8):
    ...
    while True:
        try:
            ...
            if sum(1 for v in cal if v < INTERSECTION_THRESHOLD) == 8:
                ...
                turn = 0
                t_cooldown = 40
                evaluating_intersection = False  # Resetear en caso de estar evaluando
                continue
            
            # --- EVALUACIÓN ACTIVA DE LA VENTANA DESLIZANTE ---
            if evaluating_intersection:
                eval_cycles_left -= 1
                
                # Verificamos si los canales centrales (3 y 4) están blancos (> 65)
                # lo cual indicaría que la línea dobló y ya no sigue de largo.
                if cal[3] >= 65 and cal[4] >= 65:
                    center_lost_count += 1
                
                if eval_cycles_left == 0:
                    evaluating_intersection = False
                    # Si el centro estuvo blanco más del 60% del tiempo (15 de 25 ciclos)
                    if center_lost_count >= 15:
                        ev3.speaker.beep(300, 200)  # Pitido grave
                        if eval_type == 'LEFT':
                            print("\n[Rolling] Curva L Izquierda (90) - Girando...")
                            drive.turn(-90)
                        else:
                            print("\n[Rolling] Curva L Derecha (90) - Girando...")
                            drive.turn(90)
                    else:
                        # Confirmado T-Intersection (el centro siguió leyendo negro)
                        ev3.speaker.beep(800, 100)
                        wait(50)
                        ev3.speaker.beep(800, 100)
                        print("\n[Rolling] T Confirmada - Siguiendo derecho")
                    
                    # Reiniciamos variables después del giro
                    last_err = 0
                    integral = 0
                    turn = 0
                    t_cooldown = 40
            
            # --- DETECCIÓN DE INICIO DE VENTANA ---
            elif t_cooldown == 0:
                match_l = matches_mask(cal, T_L_MASK)
                match_r = matches_mask(cal, T_R_MASK)
                
                if match_l or match_r:
                    evaluating_intersection = True
                    eval_cycles_left = 25  # ~250ms de ventana a 10ms por ciclo
                    center_lost_count = 0
                    eval_type = 'LEFT' if match_l else 'RIGHT'
                    ev3.speaker.beep(500, 30)  # Pitido muy corto de alerta
            ...
            if viz_counter >= 10:
                ...
                if pos is None:
                    if loss_counter > 8:
                        ...
                        info_str = "[ PERDIDO ]"
                elif evaluating_intersection:
                    info_str = "[EVAL:{} L:{}]".format(eval_type[0], eval_cycles_left)
                else:
                ...
                # Pantalla EV3
                ev3.screen.clear()
                ev3.screen.draw_text(10, 20, "ROLL M: CD:" + str(t_cooldown))
...
# --- MENÚ DE INICIO ---
ev3.screen.clear()
ev3.screen.draw_text(0, 10, "INICIO (ROLL)")
